...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .btn-add-cart{display:inline-flex; align-items:center; gap:8px; background:var(--ink); color:#fff; font-weight:900; text-decoration:none; padding:13px 20px; border-radius:10px; border:3px solid var(--ink); box-shadow:var(--shadow); font-size:15px; cursor:pointer; font-family:inherit;}
  .btn-add-cart:hover{background:#333;}
  .btn-add-cart:disabled{background:#ccc; border-color:#ccc; color:#888; cursor:not-allowed; box-shadow:none;}
...
<body>
  ...
  <main>
    <div class="breadcrumb">
      ...
      <a class="cat-btn" href="../../index.html" title="回首頁看更多「${escapeHtml(p.cat)}」商品">${themeIcon(p.cat)} ${escapeHtml(p.cat)}</a>
      ／ ${escapeHtml(p.sku)}
    </div>
    <div class="card">
  ...
  <script>
    ...
    if (addBtn) {
      addBtn.addEventListener('click', () => {
        ...
        else { cart[sku] = { sku, name, price, qty: 1 }; }
        saveCart(cart);
        refreshCartBadge();
